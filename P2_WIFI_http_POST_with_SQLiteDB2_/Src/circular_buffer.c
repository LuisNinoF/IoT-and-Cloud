/*
 * This code implements interrupt-driven UART communication using circular buffers:
 *
 * It maintains separate circular buffers for RX and TX data for two UART ports (slave device and debug).
 * On initialization, it enables RX interrupts so incoming bytes are stored asynchronously into RX buffers by ISR callbacks.
 * The application writes data to send into the TX buffers using buffer_write, which triggers the TX interrupt.
 * The ISR handles UART interrupts, reading incoming bytes into RX buffers and transmitting bytes from TX buffers automatically until empty.
 *
 * This design enables efficient, non-blocking serial communication
 * by separating hardware interaction (in interrupts) and data handling (in main code) with circular buffers.
 */

#include "circular_buffer.h"
#include <string.h>

#define CR1_RXNEIE		(1U<<5)		/*Receive Data Register Not Empty Interrupt Enable*/
#define CR1_TXEIE		(1U<<7)		/*Transmit Data Register Empty Interrupt Enable*/
...
/*Function to check if there is data in the buffer, and return the data available to consume*/
...
/*Function to get specified number of characters from buffer, after a given string*/
